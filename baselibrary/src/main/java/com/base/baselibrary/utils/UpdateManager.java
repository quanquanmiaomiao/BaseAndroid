...
import com.base.baselibrary.view.dialog.BaseDialog;
        ...
        OkGo.<File>get(url)
                .execute(new FileCallback() {
                    ...
                    public void downloadProgress(Progress progress) {
                        //实时更新通知栏进度条
                        int current = (int) progress.currentSize;
                        int total = (int) progress.totalSize;
                        if (type == 0) {
                            mBuilder.setProgress(total, current, false);
                            mNotifyManager.notify(10086, mBuilder.build());
                        } else if (type == 2) {
                            progressDialog.setProgress((current * 100 / total));
                        }
                    }
                });
    }
